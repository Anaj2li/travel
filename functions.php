...
function theme_scripts() {
    ...
    wp_localize_script('theme-script', 'screenReaderText', array(
        'expand' => __('expand child menu', 'site3'),
        'collapse' => __('collapse child menu', 'site3'),
    ));
...
function theme_widgets_init2() {
    register_sidebar(array(
        'name'          => __('Primary Widget Area', 'site3'),
        'id'            => 'primary',
        'description'   => __('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'site3'),
...
function _theme_get_excerpt($args = array()) {
	...
	$more_tag                     = isset($args['more_tag'])                     ? $args['more_tag']                     : __('Read more', 'site3');
...
function theme_get_category_list() {
	return str_replace(
		'<a',
		'<a class="u-textlink"',
		get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'site3'))
	);
}

function theme_get_tags_list() {
    return get_the_tag_list('', _x( ', ', 'Used between list items, there is a space after the comma.', 'site3'));
}
...
function theme_layout_before($template_key, $default_layout = '') {
    if (is_singular('post')) {
        ...
        $opt = 'product';
    } else if (function_exists('wc_get_product') && is_shop()) {
        $opt = 'products';
...
function theme_layout_after() {
    global $theme_current_layout;
    if (function_exists('wc_get_product') && (is_product() || is_shop() || is_cart())) {
